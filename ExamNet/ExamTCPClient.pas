...
begin
      ...
      if (not FCommandProcessing) and (Assigned(FOnTimer)) then begin
         FOnTimer(Sender);
...
begin
   ...
   try
     try
        ...
        if LastCmdResult.Code = CMDCONSTCORRECTREPLYCODE then begin
...
begin
   ...
   try
     try
        ...
        if LastCmdResult.Code = CMDCONSTCORRECTREPLYCODE then begin
           if ABaseConfig=nil then  ABaseConfig := TBaseConfig.Create;
   ...
   try
     try
        ...
        begin
           ...
           if ReadLn() = CMDCONSTCORRECTREPLYCODE then Result := crOk;
        end;
     except
        on E:Exception do  begin
           if E is EIdConnClosedGracefully then
