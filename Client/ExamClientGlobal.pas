...
begin
   ...
   if Result=crOk then begin
       result := crError;
      SetLength(FModules,FBaseConfig.Modules.Count);
      for I := 0 to FBaseConfig.Modules.Count - 1 do begin
...
begin
   if Examinee.ID = EmptyStr then raise Exception.Create('考生ID不能为空');
...
begin
   ...
   try
      if FExamTCPClient.CommandGetExamineeTestFilePack(AExamineeID,ALoginType,memStream)=crOk then begin
...
begin
   ...
   if FileExists(path+'\'+dbName) then  begin
      FConnClientDB.ConnectionString:='Provider=Microsoft.Jet.OLEDB.4.0;Data Source='
...
var
   sj:string;
begin
  with TExamClientGlobal,TExamClientGlobal.Examinee do begin
    RemainTime := RemainTime-1;
    sj:=format('%.2d:%.2d',[RemainTime div 60,RemainTime mod 60]);
    {$IFDEF FLASH}
    ClientMainForm.SetFlashRemainTime(sj);
    {$ENDIF}
    //TExamClientGlobal.ClientMainForm.stTime.Caption:=sj;
    if assigned(FloatWindow) then
    begin
      Floatwindow.stTime.caption:='剩余:'+sj;
