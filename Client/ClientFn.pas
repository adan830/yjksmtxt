...
interface
uses DB,dbtables,SysUtils,Variants,excel2000,word2000,msppt2000, classes, forms,windows,Adodb,shellapi, uFnPublic;
{$DEFINE WORD2000}
 // udmMain,
type
  //选择题窗口树控件用
...
procedure SingleSelectGrade(pbHandle:integer); //单选评分
procedure MultiSelectGrade(pbHandle:integer);  //多选评分
procedure WinGrade(pbHandle:integer);          //Windows评分
procedure WordGrade(pbHandle:integer);
procedure ExcelGrade(pbHandle:integer);
procedure PptGrade(pbHandle:integer);
...
implementation
uses comobj,uwingrade, ufrmInProcess, uWordGrade,
  uPptGrade,uExcelGrade, uGrade,udmExam;
...
begin
   ...
   try
     tempStringList.Text :=dm.Filterqueryst_hj.AsWideString;
...
begin
   ...
   tempStringList:=TstringList.Create;
   try
     tempStringList.Text:=dm.Filterqueryst_hj.AsString;
     for i:=0 to tempstringlist.count-1 do  begin
...
procedure SingleSelectGrade(pbHandle:integer);
var
  fs:integer;
  str:string;
...
begin
  ...
  try
    dm.FilterQuery.Active:=false;
    dm.FilterQuery.Parameters.ParamByName('v_stno').Value:='A%';
    dm.filterquery.active:=true;
    fs:=0;
    ...
    begin
      ...
      dm.filterquery.Next;
      TProcessManager.OnProcess(pbHandle,'正在对　单项选择题　评分！');
  //    sleep(100);
    end;
...
procedure MultiSelectGrade(pbHandle:integer);
var
  fs:integer;
  str:string;
  ScoreInfoStrings:TStringList;
begin
  ...
  try
    ...
    begin
      ...
      ScoreInfoStrings.Add(str);
      dm.filterquery.Next;
      TProcessManager.OnProcess(pbHandle,'正在对　　多项选择题　　评分！',2);
      //sleep(100)
    end;
...
begin
  ...
  try
    ...
    begin
    ///这里本是用下面的语句，但在2006中不能使用，因此改为上面的语句
      gradeinfostrings.Text := tempSet.FieldByName('st_da1').AsWideString;
...
procedure GetGradeParam(commandstring: string;  var WGradeRec:WGradeRecord);
var
  temp,temp1:pchar;
  index:integer;
...
begin
  ...
  try
    ...
    begin
      ...
      try
        strlist.Text :=setTemp.FieldByName('试题策略').AsWideString;
        result := strlist;
      except
        strlist.Free;
        result := nil;
