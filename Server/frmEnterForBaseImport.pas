...
uses
  ...
  Dialogs, cxEdit, DB,
  ADODB, StdCtrls, cxGridLevel, cxGridCustomTableView, cxGridTableView, cxGridDBTableView, cxClasses, cxControls, cxGridCustomView, cxGrid, Mask,
  cxContainer, cxLabel, cxStyles, dxSkinsCore, dxSkinBlack, dxSkinBlue, dxSkinCaramel, dxSkinCoffee, dxSkinDarkRoom,
  ...
  dxSkinOffice2007Pink, dxSkinOffice2007Silver, dxSkinPumpkin, dxSkinSeven, dxSkinSharp, dxSkinSilver, dxSkinSpringTime,
...
var
  ...
  aa:string;
begin
if not (trim(edtBmk.text)='') then
begin
  with tblkwxxk do begin
    First;
    while not Eof do begin
        setExamineeBase.AppendRecord([EncryptStr(FieldByName('准考证号').AsString),EncryptStr(FieldByName('姓名').AsString)]);
        tblkwxxk.Next;
    end;
    //cdsExaminees.ApplyUpdates(0);
  end;
...
//      for i:=1 to dd do
//      begin
//        if not cdsExaminees.Locate(DFNEI_DECRYPTEDID,inttostr(stkh+i-1),[loCaseInsensitive]) then
//        begin
//          if tblkwxxk.Locate('准考证号',inttostr(stkh+i-1),[loCaseInsensitive]) then
//          begin
//            cdsExaminees.AppendRecord([EncryptStr(tblkwxxk.FieldByName('准考证号').AsString),EncryptStr(tblkwxxk.FieldByName('姓名').AsString),EncryptStr(IntToStr(ord(esNotLogined)))]);
//            cc:=cc+1;
//          end;
//        end;
//        cdsExaminees.ApplyUpdates(0);
