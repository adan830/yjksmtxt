...
begin
   Result := '无此状态';
   case AStatus of
      esNotLogined: Result:='未登录';
      esDisConnect: Result:='异常中断';
      esGradeError: Result:='交卷中断';
      esLogined: Result:='已登录';
      esAllowReExam: Result:='允许重考';
      esAllowContinuteExam: Result:='允许续考';
      esGetTestPaper: Result:='正在获取试卷';
      esExamining: Result:='考试中...';
      esGrading: Result:='正在评分...';
      esSutmitAchievement: Result:='提交成绩';
      esExamEnded: Result:='考试正常结束';
      esError: Result:='有错误';
      esAbsent: Result:='缺考';
      esCrib: Result:='作弊';
   end;
