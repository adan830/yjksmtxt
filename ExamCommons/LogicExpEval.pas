...
type
  ...
  public
    ...
    function Equals(AnExpressionEvaluator: TLogicExpEval): boolean;
    procedure Assign(SourceExpressionEvaluator: TLogicExpEval);
    procedure AssignTo(SL: TStringList);
    function Eval(const Expression: string):integer; overload;
    function Eval(const Expression: string;Values : string): Integer; overload;
    function GetParamCount(const Expression: string): integer;
...
begin
  ...
  if FNames<>nil then begin
    FreeMem(Pointer(FNames));
    FNames:=nil;
  end;
  if FValues<>nil then begin
    FreeMem(Pointer(FValues));
    FValues:=nil;
  end;
  FCount:=0;
  FCapacity:=0;
...
function TLogicExpEval.CompareValues(const v1, v2: string): integer;
// -1 : v1<v2
//  0 : v1=v2
//  1 : v1>v2
...
function TLogicExpEval.CompAtom(
...
constructor TLogicExpEval.Create;
begin
  inherited Create;
  FValues:=nil;
  FNames:=nil;
...
destructor TLogicExpEval.Destroy;
begin
  Clear;
  inherited Destroy;
end;

function TLogicExpEval.Eval(const Expression: string): integer;
//  -1 = true
//  0 = syntax error
// 1 = false
var s:string;
...
function TLogicExpEval.Eval(const Expression: string;Values : string): integer;
var
  head:PChar;
  chr : Char;
  index :Integer;
...
function TLogicExpEval.GetParamCount(const Expression: string): integer;
var
  head:PChar;
  chr : Char;
  count :Integer;
...
function TLogicExpEval.Items(Index: integer): string;
begin
  Result:=FNames[Index]+'='+FValues[Index];
end;

function TLogicExpEval.Names(Index: integer): string;
begin
  Result:=FNames[Index];
end;

function TLogicExpEval.Values(Index: integer): string;
begin
  Result:=FValues[Index];
end;

procedure TLogicExpEval.Append(const Variable, Value: string);
begin
  Variables[Variable]:=Variables[Variable]+Value;
end;

procedure TLogicExpEval.Prepend(const Variable, Value: string);
begin
  Variables[Variable]:=Value+Variables[Variable];
end;

function TLogicExpEval.EvalAtPos: string;
...
procedure TLogicExpEval.Expand;
...
begin
  ...
  GetMem(Pointer(NewValues),NewSize);
  GetMem(Pointer(NewNames),NewSize);
  ...
  if FValues<>nil then FreeMem(Pointer(FValues));
  if FNames<>nil then FreeMem(Pointer(FNames));
  FValues:=NewValues;
  FNames:=NewNames;
  FCapacity:=NewCapacity;
end;

function TLogicExpEval.IndexOfName(
...
function TLogicExpEval.GetVariables(const Name: string): string;
var i: integer;
begin
  i:=IndexOfName(Name);
...
function TLogicExpEval.IsDefined(const Name: string): boolean;
var i: integer;
begin
  i:=IndexOfName(Name);
...
function TLogicExpEval.ReadNextAtom: boolean;
var c,o1,o2:char;
begin
...
function TLogicExpEval.ReadTilEndBracket: boolean;
...
procedure TLogicExpEval.Assign(
  SourceExpressionEvaluator: TLogicExpEval);
var i, Size: integer;
begin
  Clear;
  if SourceExpressionEvaluator<>nil then begin
    ...
    GetMem(Pointer(FNames),Size);
    FillChar(Pointer(FNames)^,Size,0);
    GetMem(Pointer(FValues),Size);
...
procedure TLogicExpEval.SetVariables(const Name: string;
  const Value: string);
var i, j: integer;
begin
  i:=IndexOfName(Name);
  if (i>=0) and (i<FCount) and (CompareText(FNames[i],Name)=0) then
    ...
    FNames[i]:=UpperCase(Name);
    FValues[i]:=Value;
    inc(FCount);
  end;
end;

procedure TLogicExpEval.Undefine(const Name: string);
var i, j: integer;
begin
  i:=IndexOfName(Name);
...
function TLogicExpEval.Equals(
  AnExpressionEvaluator: TLogicExpEval): boolean;
var i: integer;
begin
...
procedure TLogicExpEval.AssignTo(SL: TStringList);
var i: integer;
begin
  if SL=nil then exit;
  SL.Clear;
...
function TLogicExpEval.AsString: string;
...
function TLogicExpEval.ConsistencyCheck: integer;
// 0 = ok
var i: integer;
begin
  ...
  for i:=0 to FCount-1 do begin
    if not (UpperCase(FNames[i])=FNames[i]) then begin
      Result:=-6;  exit;
    end;
