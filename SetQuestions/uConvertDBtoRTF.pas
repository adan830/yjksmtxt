...
begin
  ...
  try
      ...
      begin
        ...
        begin
          ...
          try
            ...
            begin
              if not(LeftStr(FieldByName('st_no').AsString, 1) = 'C') then
              begin
    ...
    on e: Exception do
    begin
      Application.MessageBox(PChar(Format('转换出错，%s', [e.Message])), PChar('错误'),
...
begin
  ...
  case AEnvironmentItem.ID of
    ...
    3:
      begin
        try
          if AExamTcpClient.CommandGetEQFile(AEnvironmentItem.Value1, memStream)
            = crOk then
          begin
